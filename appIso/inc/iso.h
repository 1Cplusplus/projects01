...
struct boot_vol_desc{
		...
		uint8_t bvd_stdId[5];
		uint8_t	bvd_version;
		uint8_t bvd_boot_sys_id[32];
		uint8_t bvd_unused[32];
		uint8_t bvd_lba_catalog[4];
		uint8_t bvd_unused2[1973];
}*bvd_p;

struct primary_vol_desc{
	uint8_t pvd_vdt;// Volume Descriptor Type
	uint8_t pvd_stdId[5];
	uint8_t	pvd_version;
...
struct supplementary_vol_desc{
	uint8_t svd_vdt; // Volume Descriptor Type
	uint8_t svd_stdId[5];
	uint8_t	svd_version;
...
char createJson(uint8_t*filesName[], uint8_t*lenIdFiles, uint32_t *lba[],uint8_t*type_file, uint8_t&count_records){
 ...
 //extern vol;
 //  dir_record *dir_record_p;
 // //extern fin_vol_desc;
 //  supplementary_vol_desc *svd_p;
